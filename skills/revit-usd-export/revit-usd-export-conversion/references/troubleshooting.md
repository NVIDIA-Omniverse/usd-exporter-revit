...
| Crash or unexplained failure | Plugin/native error | Collect log under `%USERPROFILE%\.revit_usd_export_plugin\logs\...` and note plugin, Revit, and Windows versions. |

For mapping semantics rather than failure symptoms, read `docs/concept_mapping.md`.
If the plugin is missing or will not build, switch to `../../revit-usd-export-getting-started/SKILL.md`.
