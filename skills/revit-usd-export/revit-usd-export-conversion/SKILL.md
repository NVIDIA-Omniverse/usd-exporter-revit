---
name: revit-usd-export-conversion
description: Use when a user asks to export Autodesk Revit models to USD/USDA/USDC with the NVIDIA OpenUSD Exporter Plugin for Revit, choose export settings, validate outputs, or troubleshoot exported USD.
version: "0.1.0"
...
# Conversion - NVIDIA OpenUSD Exporter Plugin for Revit

## Purpose

Guide users to export Revit models to OpenUSD through the plugin UI in a live Revit session (or document in-process `RevitUsdExportSdk` usage for custom Revit hosts), including single-view and batch exports. Choose export settings from user intent, verify outputs, and troubleshoot export results.
...
For installation, builds, plugin install, or first-run smoke tests, load `../revit-usd-export-getting-started/SKILL.md`.

## Prerequisites

- Matching Autodesk Revit version with the plugin installed, or a custom in-process Revit host using `RevitUsdExportSdk` (SDK calls require a live `UIApplication`).
...
4. Instruct the user to export through the Omniverse ribbon in Revit (single-view or batch). For SDK integrators only, document `RevitUsdExportSdk.Exporter` APIs that run inside a Revit host.
...
- Plugin UI: Omniverse ribbon export commands in Revit (primary end-user path).
- SDK entry point: `source/RevitUsdExportSDK/Exporter.cs` (`ExportView` / `ExportBatch` require live `UIApplication`; in-process Revit hosts only).
- Settings model: `source/RevitUsdExportSDK/RevitUsdExportSettings.cs`.
- Product docs: [Revit documentation](https://docs.omniverse.nvidia.com/connect/latest/revit.html)
- Per-model settings path: `%USERPROFILE%\Documents\Omniverse\Revit\<model name>\settings.json`, or inside the Revit file when 1-Click Export is enabled.
- Logs: `%USERPROFILE%\.revit_usd_export_plugin\logs\Revit-<revit version>\RevitUsdExportPlugin-<plugin version>\<YYYYMMDD_HHMMSS>.log`
...
```markdown
Export `<model/view>` via the Omniverse ribbon in Revit.
...
```markdown
Exported `<model/view>` to `<output>` with the Revit OpenUSD exporter.
