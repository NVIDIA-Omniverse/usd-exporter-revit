...
| Setup executable fails or add-in missing after install | Setup not run elevated, wrong Revit version, or stale add-in | Run `RevitUsdExportSetup<ver>.exe` and accept UAC; confirm the matching Revit version; reinstall after clean build. |
| Omniverse ribbon / export commands missing | Add-in not registered for that Revit year | Re-run setup for the installed Revit version, then relaunch Revit. |
...
If installation succeeds but exported USD output is wrong, switch to `../../revit-usd-export-conversion/SKILL.md`.
