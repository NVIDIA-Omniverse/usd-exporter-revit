---
name: revit-usd-export
description: Use when a user asks to install, build, package, document, troubleshoot, modify, or run the NVIDIA OpenUSD Exporter Plugin for Revit to export Autodesk Revit models to OpenUSD.
version: "0.1.0"
license: Apache-2.0 AND CC-BY-4.0
metadata:
  author: "NVIDIA Corporation"
  tags: [omniverse, openusd, revit, connector, exporter]
...
The NVIDIA OpenUSD Exporter Plugin for Revit is a Windows Revit add-in, reusable C# SDK, native OpenUSD bindings, installer, and test harness for exporting Autodesk Revit models to OpenUSD for Omniverse and other OpenUSD ecosystem applications.
...
- For installation, build setup, plugin install, or first-run smoke tests, read `revit-usd-export-getting-started/SKILL.md`.
- For USD export, export settings selection, concept mapping, output verification, or export troubleshooting, read `revit-usd-export-conversion/SKILL.md`.
- For code changes, docs updates, tests, packaging, or contribution workflow, read `revit-usd-export-contributing/SKILL.md`.

## Use This Skill For

- Install, verify, build, or troubleshoot the Revit OpenUSD exporter plugin.
- Guide users to export Revit 2024/2025/2026 models to `.usd`, `.usda`, or `.usdc` through the Omniverse ribbon in Revit, or document in-process `RevitUsdExportSdk.Exporter` for custom Revit hosts.
...
2. No product CLI or headless conversion. Export always runs inside a live Autodesk Revit session with the plugin (Omniverse ribbon) or another in-process Revit host calling `RevitUsdExportSdk` (`Exporter.ExportView` / `ExportBatch` require `UIApplication`). Agents cannot convert `.rvt` files for the user; guide the user to export from Revit. `RevitTestHarness` launches `Revit.exe` over IPC for integration tests only — not an end-user conversion CLI.
