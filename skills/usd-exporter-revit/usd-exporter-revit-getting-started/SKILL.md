---
name: usd-exporter-revit-getting-started
description: Use when a user asks to install, build, set up RevitDevKit, install the plugin, troubleshoot installation, or smoke-test the usd-exporter-revit.
version: "0.1.0"
...
# Getting Started - usd-exporter-revit
...
Install and verify the usd-exporter-revit for a target Revit version. Prefer a published NGC download for end-user install; use a local source build and the generated installer when changing this repository or when a published build is unavailable.
...
For export settings, concept mapping, or export troubleshooting, load `../usd-exporter-revit-conversion/SKILL.md`.
...
7. After install, smoke-test and real exports run inside live Revit via the USD Exporter ribbon (see root skill Core Constraints).
...
After running the setup installer from GitHub, binaries live under `C:\Program Files\Omniverse\UsdExporterRevit<year>\` and Revit loads the plugin via `C:\ProgramData\Autodesk\Revit\Addins\<year>\UsdExporterRevit.addin`.
...
2. Confirm the USD Exporter ribbon / OpenUSD export commands appear.
...
After the plugin is installed and the smoke test passes, move to `../usd-exporter-revit-conversion/SKILL.md` for export settings, concept mapping, verification, and export troubleshooting.
