...
internal class Prim
{
    ...
    {
        StageId = stageId;
        Name = usd.exporter.revit.core.getValidPrimName(stageId, name);
        ...
        if (parent != null && parent.Children.Any(c => c.Name == Name))
        {
            ...
            string[] uniqueNames = usd.exporter.revit.core.getValidPrimNames(stageId, new string[] { name }, reservedNames);
    ...
    public virtual void Write(long stageId)
    {
        if (Active)
        {
            ...
            {
                usd.exporter.revit.core.setDisplayName(stageId, Path, DisplayName);
            }
            // set kind
            switch (Kind)
            {
    ...
    private static string ResolveUniqueSiblingName(long stageId, string name, string[] reservedNames)
    {
        ...
        string candidate = usd.exporter.revit.core.getValidPrimName(stageId, name);
        int suffix = 1;
        while (reservedNames.Contains(candidate))
        {
            candidate = usd.exporter.revit.core.getValidPrimName(stageId, name + "_" + suffix);
            suffix++;
        }
        return candidate;
    }
