...
internal class Xform : Prim
{
    ...
    private usd.exporter.revit.GfVec3d pivot = new usd.exporter.revit.GfVec3d(0.0, 0.0, 0.0);
    ...
    public override void Write(long stageId)
    {
        if (Children.Count > 0 || !string.IsNullOrEmpty(ReferencePath))
        {
            ...
            if (!exists)
            {
                usd.exporter.revit.core.defineXform(stageId, Path);
            }
            if (matrix != null)
            {
                usd.exporter.revit.core.setLocalTransformPivot(stageId, Path, matrix, pivot);
            }
            if (bimData.Count > 0)
            {
                ...
                {
                    string valid = usd.exporter.revit.core.getValidPrimName(stageId, param.Name);
    ...
    {
        pivot = new usd.exporter.revit.GfVec3d(point.X, point.Y, point.Z);
    }
