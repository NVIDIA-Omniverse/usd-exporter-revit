...
internal class Mesh : Prim
{
    ...
    private void bindMaterial()
    {
        if (ExportManager.Settings.Options.InstanceFamilies && ExportManager.Settings.Options.FamilyInstanceStyle == FamilyInstancingStyle.InternalClasses && Parent.Parent is ClassPrim)
        {
            ...
            usd.exporter.revit.core.bindMaterial(StageId, Path, material.Path);
        }
        else
        {
...
internal class MeshData
{
    ...
    public usd.exporter.revit.GfVec3f[] GetPointsArray()
    {
        List<usd.exporter.revit.GfVec3f> points = new List<usd.exporter.revit.GfVec3f>();
        foreach (XYZ p in this.Points)
        {
            points.Add(new usd.exporter.revit.GfVec3f((float)p.X, (float)p.Y, (float)p.Z));
        }
        return points.ToArray();
    }
    public usd.exporter.revit.GfVec3f[] GetNormalsArray()
    {
        List<usd.exporter.revit.GfVec3f> normals = new List<usd.exporter.revit.GfVec3f>();
        foreach (XYZ n in this.Normals)
        {
            normals.Add(new usd.exporter.revit.GfVec3f((float)n.X, (float)n.Y, (float)n.Z));
        }
        return normals.ToArray();
    }

    public usd.exporter.revit.GfVec2f[] GetUVsArray()
    {
        List<usd.exporter.revit.GfVec2f> uvs = new List<usd.exporter.revit.GfVec2f>();
        foreach (UV uv in this.UVs)
        {
            uvs.Add(new usd.exporter.revit.GfVec2f((float)uv.U, (float)uv.V));
        }
        return uvs.ToArray();
    }
