...
public partial class App : Application
{
    ...
    private RevitInstall uninstall()
    {
        ...
        {
            if (!(file.Name.StartsWith("RevitUsdExport") || file.Name.StartsWith("RevitOmni")))
            {
                continue;
            }
        ...
        {
            result.Result = "Uninstalled";
            result.Message = "No matching RevitUsdExport/RevitOmni .addin to remove";
        }
    ...
    private RevitInstall install()
    {
        ...
        {
            string addinPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), $@"Autodesk\Revit\Addins\{version}\RevitUsdExportPlugin.addin");
    ...
    private string writeAddin(int version, string path)
    {
        try
        {
            ...
            name.Value = "OmniverseApplication";
            XElement assembly = new XElement("Assembly");
            assembly.Value = Path.Combine(ovRevitPackage, $@"lib\RevitUsdExportPlugin{version}.dll");
            ...
            fullClassName.Value = "RevitUsdExportPlugin.App";
