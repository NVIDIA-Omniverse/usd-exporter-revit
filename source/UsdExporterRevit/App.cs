...
public class App : IExternalApplication
{
...
#if REV2026
    private const string payloadPipeName = "UsdExporterRevitTestHarness_Payload2026";
    private const string modelPipeName = "UsdExporterRevitTestHarness_OpenModel2026";
    private const string messagePipeName = "UsdExporterRevitTestHarness_Message2026";
#endif
#if REV2025
    private const string payloadPipeName = "UsdExporterRevitTestHarness_Payload2025";
    private const string modelPipeName = "UsdExporterRevitTestHarness_OpenModel2025";
    private const string messagePipeName = "UsdExporterRevitTestHarness_Message2025";
#endif
#if REV2024
    private const string payloadPipeName = "UsdExporterRevitTestHarness_Payload2024";
    private const string modelPipeName = "UsdExporterRevitTestHarness_OpenModel2024";
    private const string messagePipeName = "UsdExporterRevitTestHarness_Message2024";
#endif
    ...
    private static UsdExporterRevitSettings TestHarnessSettings = null;
...
#if REV2026
        usd.exporter.revit.log.info("usd-exporter-revit 2026");
#endif
#if REV2025
        usd.exporter.revit.log.info("usd-exporter-revit 2025");
#endif
#if REV2024
        usd.exporter.revit.log.info("usd-exporter-revit 2024");
#endif
        setupRibbon(uiapp);
    ...
    {
        string logs = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".usd_exporter_revit", "logs");
        try
        {
    ...
    static void setupRibbon(UIControlledApplication app)
    {
        ...
        PushButtonData data = new PushButtonData("usd_exporter_revit_export", "Export View", Assembly.GetExecutingAssembly().Location, "UsdExporterRevit.ExportCommand");
        ...
        PushButtonData batchData = new PushButtonData("usd_exporter_revit_batch", "Batch", Assembly.GetExecutingAssembly().Location, "UsdExporterRevit.ExportBatchCommand");
        ...
        PushButtonData settingsData = new PushButtonData("usd_exporter_revit_settings", "Settings", Assembly.GetExecutingAssembly().Location, "UsdExporterRevit.SettingsCommand");
        ...
        PushButtonData aboutData = new PushButtonData("usd_exporter_revit_about", "About", Assembly.GetExecutingAssembly().Location, "UsdExporterRevit.AboutCommand");
        PushButton about = settingsPanel.AddItem(aboutData) as PushButton;
        about.ToolTip = "Information about the usd-exporter-revit";
    ...
    private static void MigrateExternalSettings(string oldTitle, string newTitle)
    {
        try
        {
            ...
            string oldPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), $@"Omniverse/UsdExporterRevit/{cleanOldTitle}");
            string newPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), $@"Omniverse/UsdExporterRevit/{cleanNewTitle}");
        ...
        {
            usd.exporter.revit.log.error($"usd-exporter-revit: Failed to migrate settings: {ex.Message}");
        }
    }
    ...
    {
        usd.exporter.revit.log.info("ready for payload");
...
#region TEST HARNESS
    private static void openPipes()
    {
        ...
        string enabled = Environment.GetEnvironmentVariable("USD_EXPORTER_REVIT_TEST_HARNESS");
        if (string.IsNullOrWhiteSpace(enabled) || !(enabled.Trim() == "1" || enabled.Trim().Equals("true", StringComparison.OrdinalIgnoreCase)))
        {
            return;
        }
        ...
        if (!piped)
        {
            ...
            {
                usd.exporter.revit.log.info(ex.Message);
                usd.exporter.revit.log.info("pipe connection timed out, expected if not being run by harness");
            }
        }
    }
    ...
    {
        usd.exporter.revit.log.info(e.Msg);
        if (!piped)
        {
            piped = true;
        }
        ...
        {
            UsdExporterRevitSettings settings = Newtonsoft.Json.JsonConvert.DeserializeObject<UsdExporterRevitSettings>(e.Msg);
            IsRunningBatch = true;
    ...
    public static void TaskFinished()
    {
        if (piped)
        {
            usd.exporter.revit.log.info("task completed");
    ...
    public static UsdExporterRevitSettings GetTestHarnessSettings()
    {
