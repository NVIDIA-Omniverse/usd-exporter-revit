...
{
    public static UsdExporterRevitSettings GetSettings(Document doc, out bool oneClick)
    {
        oneClick = false;
        UsdExporterRevitSettings settings = new UsdExporterRevitSettings(doc.Title);
        ...
        else
        {
            ...
            string path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), $@"Omniverse/UsdExporterRevit/{cleanTitle}");
            if (Directory.Exists(path))
            {
                ...
                if (File.Exists(settingFile))
                {
                    ...
                    settings = Newtonsoft.Json.JsonConvert.DeserializeObject<UsdExporterRevitSettings>(settingsString);
                }
            }
        }
        return settings;
    ...
    public static void SaveSettings(Document doc, UsdExporterRevitSettings settings, bool isOn)
    {
        if (isOn)
        {
    ...
    private static UsdExporterRevitSettings getInternalSettings(Document doc, bool doTransaction)
    {
        ...
        if (json != string.Empty)
        {
            ...
            return UsdExporterRevitSettings.Read(jObj, string.Empty);
        }
        return new UsdExporterRevitSettings();
    }
    ...
    private static void saveSettings(string docTitle, DataStorage e, UsdExporterRevitSettings settings, bool isOn)
    {
