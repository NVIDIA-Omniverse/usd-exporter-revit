...
internal static class CodeIntegrity
{
    ...
    {
        string skip = Environment.GetEnvironmentVariable("USD_EXPORTER_REVIT_SKIP_CODE_INTEGRITY");
        if (!string.IsNullOrWhiteSpace(skip) && (skip.Trim() == "1" || skip.Trim().Equals("true", StringComparison.OrdinalIgnoreCase)))
        {
            return true;
        }
        ...
        string[] files = {
            Path.Combine(pluginDir, $"UsdExporterRevit{RevitYear}.dll"),
            Path.Combine(pluginDir, $"UsdExporterRevitSDK{RevitYear}.dll"),
            Path.Combine(pluginDir, "usd_exporter_revit.dll"),
        };
        ...
        foreach (string path in files)
        {
            ...
            if (error != null)
            {
                TaskDialog.Show("usd-exporter-revit", "Plugin binaries failed Authenticode verification.\n\n" + error + "\n\nReinstall under Program Files, or set USD_EXPORTER_REVIT_SKIP_CODE_INTEGRITY=1 for local testing.");
                return false;
            }
        }
    ...
    private static bool StartsWithPath(string fullPath, string root)
    {
        ...
        return fullPath.StartsWith(rootFull + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase) || fullPath.Equals(rootFull, StringComparison.OrdinalIgnoreCase);
    }
