...
public static class Exporter
{
    ...
    public static void CancelExport()
    {
        if (_progress != null)
        {
            ...
            revit.log.info("Exporter: Export cancellation requested");
        }
    }
    ...
    public static bool HandledCancelled()
    {
        if (IsCancelled())
        {
            ...
            revit.log.info("Export cancelled by user");
            return true;
        }
        return false;
    }

    public static Result ExportView(UIApplication app, View3D view, RevitUsdExportSettings settings)
    {
        ...
        revit.log.info($"[PERF] Exporter.MaterialProcessing | Duration: {perfMaterialsTimer.ElapsedMilliseconds} ms | DocumentTitle: {doc.Title}");
        ...
        using (CustomExporter ce = new CustomExporter(doc, new ExportContext()))
        {
            ...
            {
                revit.log.warning($"Exception thrown during export of {view.Name}: {ex.Message}");
                revit.log.warning($"Stacktrace: {ex.StackTrace}");
            }
        }
        perfGeometryTimer.Stop();
        revit.log.info($"[PERF] Exporter.GeometryExport | Duration: {perfGeometryTimer.ElapsedMilliseconds} ms | ViewName: {view.Name}, ElementCount: {elements.Count}");
        ...
        perfWriteTimer.Stop();
        revit.log.info($"[PERF] Exporter.WriteToUsd | Duration: {perfWriteTimer.ElapsedMilliseconds} ms | DocumentTitle: {doc.Title}");
        ...
        perfTotalTimer.Stop();
        revit.log.info($"[PERF] Exporter.TotalExportTime | Duration: {perfTotalTimer.ElapsedMilliseconds} ms | ViewName: {view.Name}, DocumentTitle: {doc.Title}, ElementCount: {elements.Count}");

        return Result.Succeeded;
    }
    public static Result ExportBatch(UIApplication app, RevitUsdExportSettings settings)
    {
        ...
        {
            revit.log.warning($"No revit files found at {settings.Batch.InputFolder}");
            return Result.Cancelled;
        }
        if (string.IsNullOrEmpty(settings.Batch.OutputFolder))
        {
            revit.log.warning($"Invalid output folder for batch.. canceling");
            return Result.Cancelled;
        }
...
#region setup random family so we can close docs freely
        ...
        if (File.Exists(randomFamily))
        {
            ...
            {
#if DEBUG
                revit.log.info($"opened and activated {uid.Document.PathName}");
#endif
        ...
        for (int i = 0; i < filePaths.Count; i++)
        {
            ...
            if (IsCancelled())
            {
                revit.log.info("Batch export cancelled by user");
                return Result.Cancelled;
            }

            string rFile = filePaths[i];
#if DEBUG
            revit.log.info($"attemp to open revit model at {rFile}");
#endif
            try
            {
#if DEBUG
...
#endif
                {
                    ...
                    if (rDoc == null)
                    {
                        revit.log.info("could not open and activate model");
                    }
                    else
                    {
                        revit.log.info($"document opened: {rDoc.Document.Title}");
                        revit.log.info($"app active document: {app.ActiveUIDocument.Document.Title}");
                    }
#endif
                    ...
                    if (rDoc != null)
                    {
                        ...
                        List<string> viewNames = settings.GetStringMatches(RevitUsdExportSettingType.ViewToFind, views.Select(v => v.Name).ToList());
...
#if DEBUG
                        revit.log.info($"{viewsToExport.Count} views found in {app.ActiveUIDocument.Document.Title}");
#endif
                        ProgressUpdate update = new ProgressUpdate();
                        ...
                        foreach (View3D view in viewsToExport)
                        {
                            ...
                            c++;
#if DEBUG
                            revit.log.info($"begin work on {view.Name}");
#endif
                            app.ActiveUIDocument.ActiveView = view;
#if DEBUG
                            revit.log.info($"active view: {app.ActiveUIDocument.ActiveView.Name}");
#endif
...
#if DEBUG
                            revit.log.info($"attempting to export {view.Name}");
#endif
...
#if DEBUG
                        revit.log.info($"attempt to open and activate {startingDoc}");
#endif
                        app.OpenAndActivateDocument(startingDoc);
#if DEBUG
                        revit.log.info($"attempt to close {docToClose.Title}");
#endif
                        docToClose.Close(false);
                    }
                    else
                    {
                        revit.log.error($"Unable to open file {rFile}," + $" if exporting linked models, open a new model, " + $"revit sample model or any model not linked to successfully batch export");
                    }
                }
            }
            catch (Exception e)
            {
                revit.log.error(e.Message);
    ...
    {
        revit.log.info("========== Export Options ==========");
        revit.log.info($"Include Cameras: {options.IncludeCameras}");
        revit.log.info($"Include Lights: {options.IncludeLights}");
        revit.log.info($"Include Links: {options.IncludeLinks}");
        revit.log.info($"Include BIM Data: {options.IncludeBimData}");
        revit.log.info($"Include Rooms: {options.IncludeRooms}" + (options.IncludeRooms ? $" (Scheme: {options.RoomColorScheme})" : ""));
        revit.log.info($"Include Spaces: {options.IncludeSpaces}" + (options.IncludeSpaces ? $" (Scheme: {options.SpaceColorScheme})" : ""));
        revit.log.info($"Include Drawings: {options.IncludeDrawings}" + (options.IncludeDrawings ? $" (Publish Set: {options.DrawingPublishSet})" : ""));
        revit.log.info($"Instance Families: {options.InstanceFamilies}" + (options.InstanceFamilies ? $" (Style: {options.FamilyInstanceStyle})" : ""));
        ...
        revit.log.info($"Coordinate System: {coordinateSystemName}");
        revit.log.info($"Material Style: {options.MaterialStyle}");
        revit.log.info($"Material Folder Name: {options.MaterialFolderName}");
        revit.log.info($"Unit Type: {options.UnitType}");
        revit.log.info("====================================");
    }
