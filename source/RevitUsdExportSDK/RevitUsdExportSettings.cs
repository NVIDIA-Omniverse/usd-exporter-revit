...
#else
namespace RevitUsdExport.Settings
#endif
{
[Serializable]
public class RevitUsdExportSettings
{
...
#region Batch
    ...
    public RevitUsdExportSettings()
    {
    }

    public RevitUsdExportSettings(string fileName)
    {
    ...
    public bool Write(string filePath)
    {
        ...
        if (!revit.file.client.isLocalUri(filePath))
        {
            revit.log.warning($"Writing settings to non-local paths is not supported: \"{filePath}\"");
            return false;
        }
        else
        {
    ...
    public static RevitUsdExportSettings Read(JObject settings, string filePath)
    {
        RevitUsdExportSettings output = new RevitUsdExportSettings();
        if (settings != null)
        {
            if (settings.ContainsKey("standard_export_settings")) // old json format
            {
                RevitUsdExportSettings updated = updateFromOld(settings);
                if (string.IsNullOrEmpty(filePath))
                {
                    return updated;
                }
    ...
    public static RevitUsdExportSettings Read(string filePath)
    {
        RevitUsdExportSettings output = new RevitUsdExportSettings();
    ...
    private static RevitUsdExportSettings updateFromOld(JObject old)
    {
        RevitUsdExportSettings current = new RevitUsdExportSettings();
        JToken token = null;
    ...
    public bool IsBatchOutputFolderValid()
    {
        ...
        if (Directory.Exists(value) || revit.file.client.isLocalUri(value))
        {
            return true;
        }
        return false;
    ...
    public string GetStringMatch(RevitUsdExportSettingType setting, List<string> names)
    {
    ...
    public List<string> GetStringMatches(RevitUsdExportSettingType setting, List<string> names)
    {
        List<string> matches = new List<string>();
        string testValue = string.Empty;
        switch (setting)
        {
            case RevitUsdExportSettingType.DetailLevel:
                testValue = this.View.DetailLevel;
                break;
            case RevitUsdExportSettingType.PhaseFilter:
                testValue = this.View.PhaseFilter;
                break;
            case RevitUsdExportSettingType.ViewToFind:
                testValue = this.Batch.ViewToFind;
                break;
            case RevitUsdExportSettingType.ViewTemplate:
                testValue = this.View.ViewTemplate;
                break;
            case RevitUsdExportSettingType.PublishSet:
                testValue = this.Options.DrawingPublishSet;
                break;
            case RevitUsdExportSettingType.RoomColorScheme:
                testValue = this.Options.RoomColorScheme;
                break;
            case RevitUsdExportSettingType.SpaceColorScheme:
...
public class SingleFileExport
{
    ...
    {
        OutputFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), @"Omniverse\Revit").Replace('\\', '/');
    }
...
public class BatchExport
{
    ...
    {
        ViewToFind = "3D";
        OutputFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), @"Omniverse\Revit\Batch").Replace('\\', '/');
#if REV2026
...
public enum RevitUsdExportSettingType
{
    DetailLevel,
    PhaseFilter,
    ViewToFind,
