...
internal class Light : Prim
{
    ...
    private Light(long stageId, string name, Prim parent) : base(stageId, name + $"_{parent.Children.Count}", parent)
    {
        ...
        double scale = revit.usd.export.core.getGeomLinearUnits(Enum.GetName(typeof(UnitType), ExportManager.Settings.Options.UnitType));
        radius = radius / scale;
    ...
    public override void Write(long stageId)
    {
        ...
        switch (Shape.Type)
        {
            ...
            case UsdLightType.Rectangle:
                ...
                revit.usd.export.core.defineRectLight(stageId, Path, rectangleLight.Width, rectangleLight.Height, Intensity);
                break;
        ...
        revit.usd.export.core.createColorTemperatureAttr(stageId, Path, Temperature);
        revit.usd.export.core.createEnableColorTemperatureAttr(stageId, Path, true);
        ...
        if (!string.IsNullOrEmpty(IesFileName))
        {
            ...
            if (!revit.file.client.isLocalUri(iesFolder))
            {
                // Skip writing the IES attribute rather than leave a dangling reference to a file we cannot copy.
                revit.log.warning($"Copying IES profiles to non-local paths is not supported; skipping IES for \"{Name}\": \"{iesFolder}\"");
            }
            else
            {
        ...
        revit.usd.export.core.setLocalTransformMatrix(stageId, Path, matrix);
        // 4. compute light extents
        revit.usd.export.core.createLightExtentAttr(stageId, Path);
        base.Write(stageId);
    }
