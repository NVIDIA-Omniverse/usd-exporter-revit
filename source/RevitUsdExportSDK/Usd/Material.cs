...
internal class Material : Prim
{
    ...
    {
#if DEBUG && MATERIALS
        revit.log.info(source.Material.Name);
#endif
    ...
    public Material(long stageId, Prim parent, string name, Color color, long materialId) : base(stageId, name, parent)
    {
        ...
        Attributes = new MaterialAttributes();
        setOmniGlass();
        Attributes.GfVec3fs.Add(OmniGlass.GlassColor, revit.usd.export.core.sRgbToLinear(new revit.usd.export.GfVec3f(color.Red / 255f, color.Green / 255f, color.Blue / 255f)));
    ...
    private Material(long stageId, Prim parent, Material material) : base(stageId, material.Name, parent)
    {
        ...
        foreach (KeyValuePair<string, revit.usd.export.GfVec2f> pair in material.Attributes.GfVec2fs)
        {
            revit.usd.export.GfVec2f v2fs = new revit.usd.export.GfVec2f(pair.Value.x, pair.Value.y);
            Attributes.GfVec2fs.Add(pair.Key, v2fs);
        }
        foreach (KeyValuePair<string, revit.usd.export.GfVec3f> pair in material.Attributes.GfVec3fs)
        {
            revit.usd.export.GfVec3f v3fs = new revit.usd.export.GfVec3f(pair.Value.x, pair.Value.y, pair.Value.z);
    ...
    public override void Write(long stageId)
    {
        if (InUse)
        {
            ...
            {
                revit.usd.export.GfVec3f color = Attributes.GfVec3fs.TryGetValue(OmniPBR.AlbedoColor, out revit.usd.export.GfVec3f albedoColor) ? albedoColor : revit.usd.export.core.sRgbToLinear(new revit.usd.export.GfVec3f(225f / 255f, 225f / 255f, 225f / 255f));
                ...
                revit.usd.export.core.defineOmniPbrMaterial(stageId, Path, color, opacity, roughness, metallic);
            }
            else if (this.AssetPath == OmniGlass.Path)
            {
                revit.usd.export.GfVec3f color = Attributes.GfVec3fs.TryGetValue(OmniGlass.GlassColor, out revit.usd.export.GfVec3f glassColor) ? glassColor : new revit.usd.export.GfVec3f(1.0f, 1.0f, 1.0f);
                ...
                roughness = clamp(roughness);
                revit.usd.export.core.defineOmniGlassMaterial(stageId, Path, color, ior, roughness);
            }
            else
            {
                revit.usd.export.core.createMaterial(stageId, Parent.Path, Name);
                revit.usd.export.core.createMdlShader(stageId, Path, "MdlShader", AssetPath, Module, true);
            }
            if (exists)
            {
    ...
    private void processSource(MaterialSource source)
    {
        ...
        if (isGlass() || source.Material.Transparency > 10)
        {
            ...
            try
            {
                this.Attributes.GfVec3fs.Add(OmniGlass.GlassColor, new revit.usd.export.GfVec3f(color.Red / 255f, color.Green / 255f, color.Blue / 255f));
            }
            catch (Exception e)
            {
                revit.log.warning(e.Message);
            }
        }
        else
        {
            ...
            try
            {
                this.Attributes.GfVec3fs.Add(OmniPBR.AlbedoColor, new revit.usd.export.GfVec3f(color.Red / 255f, color.Green / 255f, color.Blue / 255f));
            }
            catch (Exception e)
            {
                revit.log.warning(e.Message);
    ...
    private void setUV(Asset asset)
    {
        ...
        {
            this.Attributes.GfVec2fs.Add(OmniGlass.TextureTranslate, new revit.usd.export.GfVec2f((float)transX, (float)transY));
            this.Attributes.GfVec2fs.Add(OmniGlass.TextureScale, new revit.usd.export.GfVec2f((float)scaleX, (float)scaleY));
        }
        else
        {
            this.Attributes.GfVec2fs.Add(OmniPBR.TextureTranslate, new revit.usd.export.GfVec2f((float)transX, (float)transY));
            this.Attributes.GfVec2fs.Add(OmniPBR.TextureScale, new revit.usd.export.GfVec2f((float)scaleX, (float)scaleY));
        }
    }
    ...
    private void getGenericAttributes(Asset asset)
    {
        ...
        if (isGlass() || transparency > 0)
        {
            ...
            if (tintOn)
            {
                this.Attributes.GfVec3fs.Add(OmniGlass.GlassColor, new revit.usd.export.GfVec3f(tint.Red / 255f, tint.Green / 255f, tint.Blue / 255f));
            }
        }
        else
        {
            ...
            if (tintOn)
            {
                this.Attributes.GfVec3fs.Add(OmniPBR.AlbedoTint, new revit.usd.export.GfVec3f(tint.Red / 255f, tint.Green / 255f, tint.Blue / 255f));
            }
    ...
    private void getPrismOpaqueAttributes(Asset asset)
    {
        ...
        if (emissive)
        {
            ...
            this.Attributes.GfVec3fs.Add(OmniPBR.EmissiveColor, new revit.usd.export.GfVec3f(emissiveColor.Red / 255f, emissiveColor.Green / 255f, emissiveColor.Blue / 255f));
        }
    }
    ...
    private void getPrismGlazingAttributes(Asset asset)
    {
        ...
        this.Attributes.GfVec3fs.Add(OmniGlass.ReflectionColor, new revit.usd.export.GfVec3f(reflectionColor.Red / 255f, reflectionColor.Green / 255f, reflectionColor.Blue / 255f));
    ...
    private void getMirrorAttributes(Asset asset)
    {
        ...
        this.Attributes.GfVec3fs.Add(OmniGlass.ReflectionColor, new revit.usd.export.GfVec3f(color.Red / 255f, color.Green / 255f, color.Blue / 255f));
    }
    ...
    private static revit.usd.export.GfVec3f getColorFromTemp(double temperature)
    {
        float red = 0.0f;
        float green = 0.0f;
        float blue = 0.0f;
        ...
        return new revit.usd.export.GfVec3f(red, green, blue);
    }
}
...
internal class MaterialAttributes
{
    ...
    public Dictionary<string, revit.usd.export.GfVec2f> GfVec2fs = new Dictionary<string, revit.usd.export.GfVec2f>();
    public Dictionary<string, revit.usd.export.GfVec3f> GfVec3fs = new Dictionary<string, revit.usd.export.GfVec3f>();
    ...
    public void Write(long stageId, string materialPath)
    {
        ...
        foreach (var kvp in Booleans)
        {
            ...
            revit.usd.export.core.createMdlShaderInputBool(stageId, materialPath, kvp.Key, kvp.Value);
        }
        foreach (var kvp in Integers)
        {
            revit.usd.export.core.createMdlShaderInputInt(stageId, materialPath, kvp.Key, kvp.Value);
        }
        foreach (var kvp in Floats)
        {
            ...
            revit.usd.export.core.createMdlShaderInputFloat(stageId, materialPath, kvp.Key, kvp.Value);
        }
        foreach (var kvp in GfVec2fs)
        {
            revit.usd.export.core.createMdlShaderInputFloat2(stageId, materialPath, kvp.Key, kvp.Value);
        }
        foreach (var kvp in GfVec3fs)
        {
            ...
            revit.usd.export.core.createMdlShaderInputColor3f(stageId, materialPath, kvp.Key, revit.usd.export.core.sRgbToLinear(kvp.Value));
        }
        foreach (var kvp in Assets)
        {
            switch (kvp.Key)
            {
                case OmniPBR.AlbedoMap:
                    revit.usd.export.core.addDiffuseTextureToPbrMaterial(stageId, materialPath, kvp.Value);
                    break;
                case OmniPBR.NormalMap:
                    revit.usd.export.core.addNormalTextureToPbrMaterial(stageId, materialPath, kvp.Value);
                    break;
                case OmniPBR.OpacityMap:
                    revit.usd.export.core.addOpacityTextureToPbrMaterial(stageId, materialPath, kvp.Value);
                    break;
                case OmniPBR.RoughnessMap:
                    revit.usd.export.core.addRoughnessTextureToPbrMaterial(stageId, materialPath, kvp.Value);
                    break;
                case OmniPBR.MetallicMap:
                    revit.usd.export.core.addMetallicTextureToPbrMaterial(stageId, materialPath, kvp.Value);
                    break;
                case OmniPBR.OrmMap:
                    revit.usd.export.core.addOrmTextureToPbrMaterial(stageId, materialPath, kvp.Value);
                    break;
                case OmniGlass.GlassMap:
                    revit.usd.export.core.createMdlShaderInputAsset(stageId, materialPath, kvp.Key, kvp.Value, revit.usd.export.ColorSpace.eSrgb);
                    break;
                default:
                    revit.usd.export.core.createMdlShaderInputAsset(stageId, materialPath, kvp.Key, kvp.Value, revit.usd.export.ColorSpace.eRaw);
                    break;
            }
        }
    }
    ...
    private bool writeEmissive(long stageId, string materialPath)
    {
        ...
        revit.usd.export.GfVec3f color = GfVec3fs.TryGetValue(OmniPBR.EmissiveColor, out revit.usd.export.GfVec3f emissiveColor)
            ? emissiveColor
            : new revit.usd.export.GfVec3f(1.0f, 1.0f, 1.0f);
        float intensity = Floats.TryGetValue(OmniPBR.EmissiveIntensity, out float emissiveIntensity) ? emissiveIntensity : 0.0f;
        return revit.usd.export.core.addEmissiveColorToPbrMaterial(stageId, materialPath, revit.usd.export.core.sRgbToLinear(color), intensity);
    }
}
