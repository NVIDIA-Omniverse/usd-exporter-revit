...
internal class ExportContext : IExportContext, IDisposable
{
    ...
    public RenderNodeAction OnElementBegin(ElementId elementId)
    {
        ...
        if (_element is CurveElement)
        {
            ...
            if (curve is Arc)
            {
                revit.log.warning($"Arc curve is unsupported, skipping element {_element.Id.GetValue()}");
                return RenderNodeAction.Skip;
            }
        }
        ...
        revit.log.info($"{elementId.GetValue()} {_element.Category.Name} {_element.Name}");
#endif
        if (inLink)
        {
    ...
    public void OnElementEnd(ElementId elementId)
    {
        ...
        {
#if DEBUG && EXPORT_CONTEXT
            revit.log.info("Element already cleared, probably was a link");
#endif
            return;
        }
#if DEBUG && EXPORT_CONTEXT
        revit.log.info($"{elementId.GetValue()}");
#endif
        if (_ec.Count > 0)
        {
#if DEBUG && EXPORT_CONTEXT
            revit.log.info(_ec.Last().ToString());
#endif
    ...
    public RenderNodeAction OnInstanceBegin(InstanceNode node)
    {
...
#if DEBUG && EXPORT_CONTEXT
        revit.log.info($"Type Id: {_typeId.GetValue()}");
        revit.log.info($"Symbol Id: {_symbolId.GetValue()} <-- this one can vary from the type id!");
        FamilySymbol fSymbol = _doc.GetElement(_symbolId) as FamilySymbol;
        object import = _doc.GetElement(_symbolId);
        revit.log.info($"symbol id element type: {import.GetType()}");
        if (fSymbol != null)
        {
            revit.log.info($"Family Id:{fSymbol.Family.Id.GetValue()}");
        }
        else
        {
            revit.log.info($"Family Id: NULL");
        }
        revit.log.info("instance node transform:");
        t.Log();
        revit.log.info("multiplied transform:");
        multi.Log();
#endif
        if (ExportManager.Settings.Mappings.FamilyTypes.UserMapped.Any(f => f.Id == typeId.GetValue()))
        {
            ...
            if (revit.file.client.isLocalUri(map.AssetPath))
            {
                assetMetersPerUnit = revit.usd.export.core.getMetersPerUnitFromFile(map.AssetPath);
            }
...
#if DEBUG
                revit.log.info("scaled transform");
                scale.Log();
#endif
            }
...
#if DEBUG
                revit.log.info("rotation transform");
                rotation.Log();
                revit.log.info("point transform");
                transform.Log();
                revit.log.info("final transform");
                final.Log();
#endif
    ...
    {
#if DEBUG && EXPORT_CONTEXT
        revit.log.info($"Instance End: {node.GetSymbolGeometryId().SymbolId.GetValue()}");
#endif
    ...
    public void OnLight(LightNode node)
    {
        ...
        if (ExportManager.Settings.Options.IncludeLights)
        {
            ...
            {
                revit.log.warning($"Could not convert Element {_element.Id.GetValue()} to LightType, please consider rebuilding this light with a newer Revit template");
                revit.log.warning($"Failed to process Light, creating default sphere light: {ex.Message}");
            }
            if (lightType == null)
            {
    ...
    {
#if DEBUG && EXPORT_CONTEXT
        revit.log.info($"Link {_element.Name}");
#endif
...
#if DEBUG && EXPORT_CONTEXT
        revit.log.info($"{_materialId.GetValue()}");
#endif
    ...
    public void OnPolymesh(PolymeshTopology node)
    {
        ...
        for (int i = 0, vPos = 0; i < nodeFacets.Count; ++i, vPos += 3)
        {
            ...
            {
                revit.log.warning($"The vertex index specification for a mesh face is invalid: {_element.Name} {_prim.DisplayName}");
                break;
            }
        }
        for (int i = 0; i < faceVertexIndices.Count; ++i)
        {
            ...
            {
                revit.log.warning($"Vertex index out of range ({faceVertexIndices[i]}, NumberOfPoints = {NumberOfPoints}): {_element.Name} {_prim.DisplayName}");
                break;
            }
        }
#endif
