...
internal class Drawing
{
    ...
    public Drawing(Prim parent, View view)
    {
            ...
            using (Transaction t = new Transaction(view.Document))
            {
                if (t.Start("adding temporary line") == TransactionStatus.Started)
                {
                    ...
                    for (int i = 0; i < _size; i++)
                    {
                        ...
                        {
                            revit.log.error($"[{view.Name}] {e.Message}");
                        }
                    }
                }
                t.Commit();
        ...
        {
            revit.log.warning($"skipping drawing export of View {view.Name} {view.GetType().Name}");
            revit.log.warning($"views must be 2D and have an active crop box to be exported");
        }
    }
    public static void ExportDrawings(Prim parent)
    {
        ...
        if (viewSheetSets == null || viewSheetSets.Count == 0)
            return;
        string publishSetName = ExportManager.Settings.GetStringMatch(RevitUsdExportSettingType.PublishSet, viewSheetSets.Select(v => v.Name).ToList());
    ...
    private static XYZ[] getRegistrationMarks(string path, out string filePathWithExtension, out XYZ bitmapSize)
    {
        ...
        if (file != null && file.Exists)
        {
            ...
            {
                revit.log.error($"Exception while processing bitmap: {e.Message}");
                bitmapSize = XYZ.Zero;
            }
        }
        else
