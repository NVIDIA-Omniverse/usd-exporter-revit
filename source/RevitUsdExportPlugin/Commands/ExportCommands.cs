...
public class ExportCommand : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        bool oneClick = false;
        RevitUsdExportSettings settings = Storage.GetSettings(doc, out oneClick);
        ...
        if (!(oneClick && settings.IsOutputFolderValid()))
        {
            ...
            settings = Newtonsoft.Json.JsonConvert.DeserializeObject<RevitUsdExportSettings>(result.SettingsString);
        ...
        else
        {
            ...
            foreach (string viewName in settings.ViewsToExport)
            {
                ...
                {
                    Dialogs.ProgressDialogStatus -= OnProgressDialogClosed;
                    revit.log.warning("View does not exist.");
                    return Result.Cancelled;
                }
                ...
                if (view != null)
                {
                    ...
                    {
                        revit.log.error($"Export of {viewName} failed: {ex.Message}");
                        continue;
                    }
                ...
                {
                    revit.log.warning($"Unable to find {viewName} for export...");
                }
            }
            r = Result.Succeeded;
        ...
        Dialogs.ProgressDialogStatus -= OnProgressDialogClosed;
        revit.log.info($"Export Completed: Thread ID: {Thread.CurrentThread.ManagedThreadId}, Timestamp: {DateTime.Now}");
...
public class ExportBatchCommand : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        bool oneClick = false;
        RevitUsdExportSettings settings = new RevitUsdExportSettings();
        List<View3D> views = new List<View3D>();
        Dictionary<string, bool> viewMap = GetInput.ThreeDViews(doc, out views);
        List<ColorFillScheme> roomSchemes = GetInput.RoomSchemes(doc);
        List<ColorFillScheme> spaceSchemes = GetInput.SpaceSchemes(doc);
        ...
        if (!(settings.IsInputFolderValid() && settings.IsBatchOutputFolderValid()))
        {
            ...
            revit.log.error($"INVALID FOLDERS FOR BATCH \"{settings.Batch.InputFolder}\" or \"{settings.Batch.OutputFolder}\"");
            return Result.Cancelled;
        }
    ...
    {
        revit.log.info($"ExportBatchCommand: Progress dialog closed with state: {context.State}");
...
internal static class GetInput
{
    ...
    public static List<MaterialData> Materials(Document doc, RevitUsdExportSettings settings)
    {
    ...
    public static List<FamilyData> Families(Document doc, RevitUsdExportSettings settings)
    {
